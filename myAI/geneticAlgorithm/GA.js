 ...
 * @Description: 遗传算法 GA 解决简单的负载均衡问题

 * 假设有N个任务，需要负载均衡器分配给M个服务器节点去处理。每个任务的任务长度、每台服务器节点(下面简称“节点”)的处理速度已知，请给出一种任务分配方式，使得所有任务的总处理时间最短。
 
 * Tasks={2,4,6,8}，节点集合为Nodes={2,1}

 * @Date: 2018-01-17 06:08:14

 * @LastEditors: krisGooooo

 * @LastEditTime: 2018-12-17 20:43:32
 */
 ...
 * GA算法重要的基础概念心得
 * 
 * 1.基因和染色体的本质：某一个数学问题的可行解 是染色体  可行解中的某一个元素 是基因 
 * 
 * 2.适应度函数： 作为上帝衡量染色体的优劣。在运行的过程中会进行N次迭代，每次迭代都会生成若干条染色体。适应度函数会给本次迭代中生成的所有染色体打个分，来评判这些染色体的适应度，然后将适应度较低的染色体淘汰掉，只保留适应度较高的染色体，从而经过若干次迭代后染色体的质量将越来越优良。
 * 
 * 3.交叉：每一次交叉生成新一轮迭代的染色体，需要从上一代染色体中寻找两条染色体，一般通过轮盘赌算法，每一轮迭代后，都会计算每一条染色体的适应度，染色体i被选择的概率 = 染色体i的适应度 / 所有染色体的适应度之和
 * 
 * 4.变异：交叉后基因还是那些，计算结果更接近于局部最优解，而永远没办法达到全局最优解，交叉后在新染色体上修改几个基因
 * 
 * 5.复制：为了保留上一代优良的染色体，需要将上一代中适应度最高的几条染色体直接原封不动地复制给下一代
 * 
 */
 ...
 * 理解
 * 
 * 每一次进化都会更优，因此理论上进化的次数越多越好，但在实际应用中往往会在结果精确度和执行效率之间寻找一个平衡点，还是要随业务场景出发。
 * 
 * 1.限制进化次数
 * 
 * 2.限定结果集允许范围
